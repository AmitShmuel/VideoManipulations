...
typedef struct VideoState {
	...
	int			 color_flag, save_picture_flag, sound_flag;
	double		 curr_pts;
...
/* Since we only have one decoding thread, 
   the Big Struct can be global in case we need it. */
...
void audio_callback(void *userdata, Uint8 *stream, int len) {
	...
	while(len > 0) {
		...
		if(is->sound_flag != 3) // muting both videos
...
void savePicture(VideoState* is, AVFrame* pFrame) {
	...
	if (file_exist(name))
		...
		fprintf(stderr,"Failed saving screenshot\n");
}

void toRGB(void *arguments) {
	
	VideoState *is = (VideoState*)arguments;
	int rgb_1,rgb_2,numBytes, small_numBytes, i;
	AVFrame *pFrameRGB = NULL, *pFrame = NULL, *smallFrame = NULL;
	uint8_t *buffer = NULL, *small_buffer;
	struct SwsContext *sws_ctx = NULL, *sws_ctx_2=NULL, *small=NULL, *small2 = NULL;

	// Allocate main video frame
	...
	// Allocate secondary video frame
	smallFrame = av_frame_alloc();
	if(smallFrame==NULL) pthread_exit(NULL);
	for(;;) // protect in case codec is NULL
		if(is->is2->video_st != NULL) 
			break; 
	AVCodecContext *small_pCodecCtx = is->is2->video_st->codec;
	// Determine required buffer size and allocate buffer
	small_numBytes=avpicture_get_size(PIX_FMT_RGB24, small_pCodecCtx->width, small_pCodecCtx->height);
	small_buffer=(uint8_t *)av_malloc(small_numBytes*sizeof(uint8_t));
	avpicture_fill((AVPicture *)smallFrame, small_buffer, PIX_FMT_RGB24,small_pCodecCtx->width, small_pCodecCtx->height);
	
	//context of secondary video to quarter of the screen
	small = sws_getContext(small_pCodecCtx->width, small_pCodecCtx->height, AV_PIX_FMT_YUV420P, 
		 	pCodecCtx->width/2, pCodecCtx->height/2, small_pCodecCtx->pix_fmt, SWS_BILINEAR, NULL, NULL, NULL);
		 	
	//context of secondary video to full screen
	small2 = sws_getContext(small_pCodecCtx->width, small_pCodecCtx->height, AV_PIX_FMT_YUV420P, 
		 	pCodecCtx->width, pCodecCtx->height, small_pCodecCtx->pix_fmt, SWS_BILINEAR, NULL, NULL, NULL);	
	...
	for(;;) {
		...
		SDL_LockMutex(is->is2->colorq_mutex);
		while(is->is2->colorq_size == 0 && !is->is2->quit) {
			SDL_CondWait(is->is2->colorq_cond, is->is2->colorq_mutex);
		}
		SDL_UnlockMutex(is->is2->colorq_mutex);
		if(is->is2->quit) {
			av_frame_free(&smallFrame);
			return;
		}
		smallFrame = is->is2->colorq[is->is2->colorq_rindex];
		
		// scaling small video to large video when multiple view is on
		if(isMultiVideos) 
			sws_scale(small,(uint8_t const * const *)smallFrame->data,
					smallFrame->linesize,0,small_pCodecCtx->height,pFrame->data,pFrame->linesize);
		
		// one full screened video
		else {
			if(is->sound_flag == 2) // secondary audio is playing, scaling small video to full screen
				sws_scale(small2,(uint8_t const * const *)smallFrame->data,
						smallFrame->linesize,0,small_pCodecCtx->height,pFrame->data,pFrame->linesize);
			/*else
				primary audio is playing, pFrame is the primary video and he is full screened, so do nothing */
		}
		...
		if((is->color_flag > 0 && is->color_flag < 5) || (is->save_picture_flag)) {
			...
			if (is->color_flag > 1 && is->color_flag < 5) {
				for (i = 0; i < pCodecCtx->height*pCodecCtx->width*3; i+=3) {
					pFrameRGB->data[0][i+rgb_1] = 0;
					pFrameRGB->data[0][i+rgb_2] = 0;
				}
			}
			// black and white, writing the average of the current pixels
			if(is->color_flag == 1) {
				for (i = 0; i < pCodecCtx->height*pCodecCtx->width*3; i+=3) {
					pFrameRGB->data[0][i] = (pFrameRGB->data[0][i] + pFrameRGB->data[0][i+1]+pFrameRGB->data[0][i+2])/ 3;
					pFrameRGB->data[0][i+1] = pFrameRGB->data[0][i];
					pFrameRGB->data[0][i+2] = pFrameRGB->data[0][i];
				}
			}
		...
		if(queue_picture(is, pFrame, is->curr_pts) < 0) 
			break;
		...
		av_frame_free(&smallFrame);
		if(++is->is2->colorq_rindex == VIDEO_PICTURE_QUEUE_SIZE) {
			is->is2->colorq_rindex = 0;
		}
		SDL_LockMutex(is->is2->colorq_mutex);
		is->is2->colorq_size--;
		SDL_CondSignal(is->is2->colorq_cond);
		SDL_UnlockMutex(is->is2->colorq_mutex);
	}
}
...
int video_thread(void *arg) {
	...
	int rc;
	pthread_t thread;
	if(!is->is_small) { // creating just one toRGB thread, with the primary VideoState
	...
	for(;;) {
		...
		if(frameFinished) {
			...
			is->curr_pts = pts; // updating pts
			is->colorq[is->colorq_windex] = av_frame_clone(pFrame); // cloning current frame and sending to color queue
...
int stream_component_open(VideoState *is, int stream_index) {
	...
	if(codecCtx->codec_type == AVMEDIA_TYPE_AUDIO) {
		// we want to open audio just for the primary video when we start
		if(!is->is_small) {
...
int decode_thread(void *arg) {
	...
	// global_video_state is set to the primary video
	if(!is->is_small)
...
int main(int argc, char *argv[]) {
	...
	if(argc < 3) {
		fprintf(stderr, "You should insert 2 file names\n");
		exit(1);
	}
	if(argc < 5) 
	...
	printf("Initializing %s on %d x %d and %s on %d x %d\n",argv[1],width,height, argv[2], width/2, height/2);
	...
	for(;;) {
		...
		case SDL_KEYDOWN:
			...
			// black & white event
			case SDLK_w:					
				if (is->color_flag == 1) is->color_flag = 0;
				else is->color_flag = 1;
				break;
			// red screen event
			case SDLK_r:					
				if (is->color_flag == 2) is->color_flag = 0;
				else is->color_flag = 2;
				break;
			// green screen event
			case SDLK_g:					
				if (is->color_flag == 3) is->color_flag = 0;
				else is->color_flag = 3;
				break;
			// blue screen event
			case SDLK_b:					
				if (is->color_flag == 4) is->color_flag = 0;
				else is->color_flag = 4;
				break;
			// clear colors
			case SDLK_c:					
				is->color_flag = 0;
			...
			case SDLK_2:
				if(is->sound_flag != 2) {
					...
					wanted_spec.userdata = is->is2;
				}
				break;
			// mute both audios
			case SDLK_3:
				if(is->sound_flag != 3) is->sound_flag = is->is2->sound_flag = 3;
				break;
			// one full screened video
			case SDLK_o:
				if(isMultiVideos != 0)
